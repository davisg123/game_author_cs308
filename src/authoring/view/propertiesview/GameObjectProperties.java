...
public class GameObjectProperties extends Properties {
	...
	public GameObjectProperties(GameObject gObj, double width, double height,
			GameHandler... handler) {
	...
	public GameObjectProperties() {
		// nullary constructor that creates empty map to generate new game
		// objects
		...
		wizardProperties.put("collision constant", new PropertyTextField(
				"CollisionConstant", "0.0"));
		wizardProperties.put("gravity constant", new PropertyTextField(
				"GravityConstant", "1.0"));
		wizardProperties
				.put("density", new PropertyTextField("Density", "1.0"));
		wizardProperties.put("volume", new PropertyTextField("Volume", "1.0"));
		wizardProperties.put("initXV", new PropertyTextField(
				"Initial X Velocity", "0"));
		wizardProperties.put("initYV", new PropertyTextField(
	...
	public void initializeProperties(Object g) {
		...
		PropertyTextField unmodifiableName = new PropertyTextField("Type: ",
				gameObject.getIdentifier().getType());
		unmodifiableName.setDisable(true);
		PropertyTextField unmodifiableID = new PropertyTextField("ID: ",
				gameObject.getIdentifier().getUniqueId());
		...
		concreteTextProperties.put(
				...
				new PropertyTextField("Y: ", Double.toString(gameObject.getY()
						+ OBJECT_Y_OFFSET)));
		concreteTextProperties.put("rotation", new PropertyTextField(
				"Rotation: ", Double.toString(gameObject.getRotation())));

		physicsProperties.put(
				"initXV",
				new PropertyTextField("Initial X Velocity", Double
						.toString(gameObject.getPhysicsBody().getVelocity()
								.getX())));
		physicsProperties.put(
				"initYV",
				new PropertyTextField("Initial Y Velocity", Double
						.toString(gameObject.getPhysicsBody().getVelocity()
								.getY())));
		physicsProperties.put(
				"mass",
				new PropertyTextField("Mass", Double.toString(gameObject
						.getPhysicsBody().getScalar("Mass").getValue())));
		physicsProperties.put(
				"collision constant",
				new PropertyTextField("CollisionConstant", Double
						.toString(gameObject.getPhysicsBody()
								.getScalar("CollisionConstant").getValue())));
		physicsProperties.put(
				"gravity constant",
				new PropertyTextField("GravityConstant", Double
						.toString(gameObject.getPhysicsBody()
								.getScalar("GravityConstant").getValue())));
		physicsProperties.put(
				"density",
				new PropertyTextField("Density", Double.toString(gameObject
						.getPhysicsBody().getScalar("Density").getValue())));
		physicsProperties.put(
				"volume",
				new PropertyTextField("Volume", Double.toString(gameObject
						.getPhysicsBody().getScalar("Volume").getValue())));
	...
	public GameObject edit(GameObject g) {
		...
		GameObject edited = new GameObject(g.getComponents(),
				inherentTextProperties.get("image").getInformation(),
				Double.parseDouble(concreteTextProperties.get("x")
						.getInformation()) - OBJECT_X_OFFSET,
				Double.parseDouble(concreteTextProperties.get("y")
						.getInformation()) - OBJECT_Y_OFFSET,
		...
		pb.addScalar(new Mass(Double.parseDouble(physicsProperties.get("mass")
				.getInformation())));
		...
		pb.addScalar(new Volume(Double.parseDouble(physicsProperties.get(
				"volume").getInformation())));
		pb.setVelocity(new Velocity(Double.parseDouble(physicsProperties.get(
				"initXV").getInformation()), Double
				.parseDouble(physicsProperties.get("initYV").getInformation())));
		// may need to change
